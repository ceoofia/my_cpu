    ...
    always_ff @(posedge clk) begin 
        if (mem_en == 1'b1 && instr_req_valid == 1'b1) begin
            if(instr_req_addr_in + 3 < MEM_WIDTH) begin // guard against out of bound reading
                ...
                instr_data_out [7:0]    <= instr_memory [instr_req_addr_in];
                instr_data_out [15:8]   <= instr_memory [instr_req_addr_in + 1];
                instr_data_out [23:16]  <= instr_memory [instr_req_addr_in + 2];
                instr_data_out [31:24]  <= instr_memory [instr_req_addr_in + 3];
            end 
            else begin
