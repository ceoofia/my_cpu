...
module Memory #(
    ...
    always_ff @(posedge clk) begin
        ...
        else if (mem_en_in && mem_rw_in && addr_req_in + 32'(mem_store_size_in) - 1 < MEM_DEPTH
            && mem_store_size_in != NO_STORE_SIZE) begin
            case (mem_store_size_in)
                STORE_SIZE_WORD: begin
                    mem[DATA_ADDR + addr_req_in + 3] <= store_data_in[31:24];
                    mem[DATA_ADDR + addr_req_in + 2] <= store_data_in[23:16];
